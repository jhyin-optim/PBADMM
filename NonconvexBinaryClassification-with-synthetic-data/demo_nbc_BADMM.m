...
problem_set = [500 500;500 1000;1000 500;1000 1000;1000 2000;2000 1000;2000 4000;4000 2000];
[m1,~] = size(problem_set);
...
for i=1:m1
    ...
    m = problem_set(i,1);
    n = problem_set(i,2);
    AT = normc(randn(n,m));  % AT=(a_1,a_2,...,a_N) and normalize the column of AT 
    A = AT';
    b = randsrc(m,1); % generates an N-by-1 random bipolar matrix. 
    ...
    if m < 2000
        tstart = tic;
        L_g = 2*norm(barAtbarA)/m;
        teig = toc(tstart);
    else
        clear opts
        opts.issym = 1;
        tstart = tic;
        L_g = 2*eigs(barAtbarA,1,'LM',opts)/m;
        teig = toc(tstart);
    end
    L_g = max(L_g,1);
    fprintf(' time for eigenvalues %g %.3e\n', teig, L_g)
    fprintf('run the program\n')
    % set parameters
    clear opts
    opts = struct();
    opts.Lg = L_g;
    ...
    fprintf(fid,'%d %d & %.3f & %d/%.3f/%.2e/%.2e & %d/%.3f/%.2e/%.2e\n & %d/%.3f/%.2e/%.2e & %d/%.3f/%.2e/%.2e & %d/%.3f/%.2e/%.2e\\\\\n', ...
        m, n, teig, iter1, Tcpu1, Finfeas1, FinobjF1, iter2, Tcpu2, Finfeas2, FinobjF2, ...
